...
def edit_user(user_id):
    ...
    if form.validate_on_submit():
        ...
        if form.password.data:
            ...
            flash(_('Password reset for user %(username)s.', username=user.username), 'warning')
...
def grant_credits(user_id):
    ...
    db.session.commit()
    flash(_('Credits updated for user %(username)s.', username=user.username), 'success')
    return redirect(url_for('admin.manage_users'))
...
def edit_group(group_id):
    ...
    if form.validate_on_submit():
        ...
        if new_owner:
            ...
            flash(_('User %(username)s not found. Owner not changed.', username=form.owner_username.data), 'danger')
